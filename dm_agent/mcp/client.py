...
class MCPClient: 
    ...
    def start(self, fetch_tools: bool = True) -> bool:
        """
        启动 MCP 服务器进程
        ...
        Args:
            fetch_tools: 是否从服务器获取工具列表。设为 False 可跳过 tools/list 调用，
                        配合静态工具定义使用以节省上下文窗口。
        ...
        try:
            ...
            # 启动子进程，强制使用 UTF-8 编码避免 Windows GBK 解码错误
            if is_windows:
                # Windows 需要 shell=True 来找到 npx 等命令
                # 对包含空格的参数加双引号，避免 shell 拆分
                quoted_args = []
                ...
                self.process = subprocess.Popen(
                    ' '.join(quoted_args),  # Windows 下使用字符串命令
            ...
            # 初始化 MCP 连接并获取工具列表
            if not self._initialize(fetch_tools=fetch_tools):
                self.stop()
                return False
            ...
            if fetch_tools:
                ...
                print(f"✅ MCP 服务器 '{self.name}' 启动成功（跳过工具列表获取）")
            return True
    ...
    def _initialize(self, fetch_tools: bool = True) -> bool:
        """
        初始化 MCP 连接并获取工具列表
        ...
        Args:
            fetch_tools: 是否调用 tools/list 获取工具列表。
                        设为 False 可跳过（配合静态工具定义使用）。
